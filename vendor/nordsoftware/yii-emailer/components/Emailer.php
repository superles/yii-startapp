...
class Emailer extends CApplicationComponent
{
     ...
     * Defaults to 'php'.
     */
    public $transportType = self::TRANSPORT_SMTP;
    /**
     ...
     */
    public $viewPath = 'webroot.themes.bootstrap.views.layouts';
    /**
     * @var string the path to the default layout file. Setting this to false means that no layout will be used.
     */
    public $defaultLayout = 'webroot.themes.bootstrap.views.layouts.micro';
    /**
        ...
        switch ($this->transportType) {
            case self::TRANSPORT_SMTP:
                $transport = Swift_SmtpTransport::newInstance()
                    ...
                    ->setUsername('les_csh@mail.ru')
                    ->setPassword('Dfcbktyrj1985');
    ...
    public function updateSession() {
        $uc=Yii::app()->getComponent('user');
        /**
         * @var WebUser $uc
         */
        $user = Yii::app()->getModule('user')->user($uc->id);

        $uc->name = $user->username;
        ...
        foreach ($userAttributes as $attrName=>$attrValue) {
            $uc->setState($attrName,$attrValue);
        }
    }
}
